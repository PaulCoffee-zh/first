...
public class MyHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
	 ...
	 * existence of subclass LinkedMyHashMap. See below for hook methods defined
	 * to be invoked upon insertion, removal and access that allow
	 * LinkedMyHashMap internals to otherwise remain independent of these
	 * mechanics. (This also requires that a map instance be passed to some
	 * utility methods that may create new nodes.)
	 *
	 ...
	 * subclass, and in LinkedMyHashMap for its Entry subclass.)
	 */
	 ...
	 * the MyHashMap or otherwise modify its internal structure (e.g., rehash).
	 * This field is used to make iterators on Collection-views of the MyHashMap
	 ...
	 * specified <tt>Map</tt>. The <tt>MyHashMap</tt> is created with default
	...
	static class MyHashMapSpliterator<K, V> {
		...
		MyHashMapSpliterator(MyHashMap<K, V> m, int origin, int fence, int est, int expectedModCount) {
			this.map = m;
			this.index = origin;
			this.fence = fence;
			this.est = est;
	...
	static final class KeySpliterator<K, V> extends MyHashMapSpliterator<K, V> implements Spliterator<K> {
	...
	static final class ValueSpliterator<K, V> extends MyHashMapSpliterator<K, V> implements Spliterator<V> {
	...
	static final class EntrySpliterator<K, V> extends MyHashMapSpliterator<K, V>
	 ...
	 * LinkedMyHashMap, but not by any other subclass. Nearly all other internal
	 * methods are also package-protected but are declared final, so can be used
	 * by LinkedMyHashMap, view classes, and HashSet.
	 */
	...
	/**
	 * Entry for Tree bins. Extends LinkedMyHashMap.Entry (which in turn extends
